...
def _sonarr_rules_config_from_settings(
    row: AppSettings,
) -> RefinerRulesConfig | None:
    """Build RefinerRulesConfig from sonarr_refiner_* fields.
    Returns None when the Sonarr pipeline is disabled."""
    if not getattr(row, "sonarr_refiner_enabled", False):
        return None
    slot = (
        getattr(row, "sonarr_refiner_default_audio_slot", "primary")
        or "primary"
    ).strip().lower()
    if slot not in ("primary", "secondary"):
        slot = "primary"
    mode = (
        getattr(row, "sonarr_refiner_subtitle_mode", "remove_all")
        or "remove_all"
    ).strip().lower()
    if mode not in ("remove_all", "keep_selected"):
        mode = "remove_all"
    pref = normalize_audio_preference_mode(
        getattr(row, "sonarr_refiner_audio_preference_mode", "preferred_langs_quality")
    )
    return RefinerRulesConfig(
        primary_audio_lang=getattr(row, "sonarr_refiner_primary_audio_lang", "") or "",
        secondary_audio_lang=getattr(row, "sonarr_refiner_secondary_audio_lang", "") or "",
        tertiary_audio_lang=getattr(row, "sonarr_refiner_tertiary_audio_lang", "") or "",
        default_audio_slot=slot,  # type: ignore[arg-type]
        remove_commentary=bool(getattr(row, "sonarr_refiner_remove_commentary", False)),
        subtitle_mode=mode,  # type: ignore[arg-type]
        subtitle_langs=parse_subtitle_langs_csv(
            getattr(row, "sonarr_refiner_subtitle_langs_csv", "") or ""
        ),
        preserve_forced_subs=bool(
            getattr(row, "sonarr_refiner_preserve_forced_subs", True)
        ),
        preserve_default_subs=bool(
            getattr(row, "sonarr_refiner_preserve_default_subs", True)
        ),
        audio_preference_mode=pref,  # type: ignore[arg-type]
    )


def _sonarr_pipeline_from_settings(
    row: AppSettings,
) -> tuple[Path, Path, Path] | tuple[None, None, None]:
    """Resolve sonarr_refiner watched/output/work folders."""
    watched = _safe_resolve_folder(
        getattr(row, "sonarr_refiner_watched_folder", "") or ""
    )
    output = _safe_resolve_folder(
        getattr(row, "sonarr_refiner_output_folder", "") or ""
    )
    if watched is None or output is None:
        return None, None, None
    work = _safe_resolve_folder(
        getattr(row, "sonarr_refiner_work_folder", "") or ""
    )
    if work is None:
        work = db_path().parent / "refiner-sonarr-work"
    return watched, output, work


async def run_scheduled_sonarr_refiner_pass(
    session: AsyncSession,
) -> dict[str, Any]:
    """Scheduled entry point for the Sonarr Refiner pipeline.
    Mirrors run_scheduled_refiner_pass for the sonarr_refiner_*
    settings."""
    row = (
        await session.execute(
            select(AppSettings).order_by(AppSettings.id.asc()).limit(1)
        )
    ).scalars().first()
    if not row or not getattr(row, "sonarr_refiner_enabled", False):
        return {"ran": False, "reason": "disabled"}
    tz = row.timezone or "UTC"
    if not in_window(
        schedule_enabled=bool(
            getattr(row, "sonarr_refiner_schedule_enabled", False)
        ),
        schedule_days=getattr(row, "sonarr_refiner_schedule_days", "") or "",
        schedule_start=getattr(row, "sonarr_refiner_schedule_start", "00:00") or "00:00",
        schedule_end=getattr(row, "sonarr_refiner_schedule_end", "23:59") or "23:59",
        timezone=tz,
    ):
        return {"ran": False, "reason": "outside_schedule"}
    return await run_sonarr_refiner_pass(session, trigger="scheduled")
...
async def run_refiner_pass(
    ...
    async with _refiner_lock:
        ...
        detail = (
            f"Refiner ({trigger}): processed={ok_c} "
            f"unchanged={noop_c} dry_run_items={dry_c} "
            f"cleanup_needed={cleanup_c} errors={err_c}"
        )
        job_lines = [detail]
        if cleanup_notes:
...
async def run_sonarr_refiner_pass(
    session: AsyncSession,
    *,
    trigger: Literal["scheduled"] = "scheduled",
) -> dict[str, Any]:
    """Run the Sonarr Refiner pipeline over its configured
    watched folder. Independent of the Radarr pipeline —
    separate lock, separate settings, separate job log rows.

    Wrong-content detection for Sonarr is not yet implemented
    and is silently skipped (wc_ctx is always None).
    """
    async with _sonarr_refiner_lock:
        t_start = utc_now_naive()
        row = (
            await session.execute(
                select(AppSettings)
        ...
        if not row or not getattr(row, "sonarr_refiner_enabled", False):
            return {"ok": True, "ran": False, "reason": "disabled"}
        cfg = _sonarr_rules_config_from_settings(row)
        if cfg is None:
            return {"ok": True, "ran": False, "reason": "disabled"}
        primary_lang = (
            getattr(row, "sonarr_refiner_primary_audio_lang", "") or ""
        ).strip()
        if not normalize_lang(primary_lang):
            logger.warning(
                "Sonarr Refiner: primary audio language is required "
                "when Sonarr Refiner is enabled."
            )
            session.add(
                JobRunLog(
                    started_at=t_start,
                    ...
                    ok=False,
                    message=_refiner_job_log_text(
                        "Sonarr Refiner: primary audio language is "
                        "required — choose a language in Refiner "
                        "settings (Sonarr / Audio)."
                    ),
                    app="sonarr_refiner",
                )
            )
            await session.commit()
            return {"ok": False, "ran": False, "error": "primary_lang_required"}
        watched_root, output_root, work_dir = _sonarr_pipeline_from_settings(row)
        if watched_root is None or output_root is None or work_dir is None:
            logger.warning(
                "Sonarr Refiner: watched folder and output folder "
            ...
            session.add(
                JobRunLog(
                    ...
                    ok=False,
                    message=_refiner_job_log_text(
                        "Sonarr Refiner: watched folder and output "
                        "folder must both be configured when Sonarr "
                        "Refiner is enabled."
                    ),
                    app="sonarr_refiner",
                )
        ...
        if not watched_root.exists() or not watched_root.is_dir():
            logger.warning(
                "Sonarr Refiner: watched folder is not a readable "
                "directory: %s", watched_root
            )
            session.add(
                JobRunLog(
                    started_at=t_start,
                    ...
                    ok=False,
                    message=_refiner_job_log_text(
                        f"Sonarr Refiner: watched folder is missing "
                        f"or not a directory: {watched_root}"
                    ),
                    app="sonarr_refiner",
                )
            )
        ...
        if not output_root.exists() or not output_root.is_dir():
            logger.warning(
                "Sonarr Refiner: output folder is not a directory: %s",
                output_root,
            )
            session.add(
                JobRunLog(
                    ...
                    ok=False,
                    message=_refiner_job_log_text(
                        f"Sonarr Refiner: output folder is missing "
                        f"or not a directory: {output_root}"
                    ),
                    app="sonarr_refiner",
                )
            )
            ...
            return {"ok": False, "ran": False, "error": "output_folder_invalid"}
        files = _gather_watched_files(watched_root)
        dry = bool(getattr(row, "sonarr_refiner_dry_run", True))
        await _reconcile_interrupted_refiner_processing_rows_before_pass()
        if not dry and work_dir.is_dir():
            import time as _time_mod
            _stale_cutoff = _time_mod.time() - REFINER_FFMPEG_TIMEOUT_S
                    ...
                    if _wf.is_file() and ".refiner." in _wf.name:
                        try:
                            if _wf.stat().st_mtime < _stale_cutoff:
                                _wf.unlink()
                                logger.info(
                                    "Sonarr Refiner: removed stale "
                                    "work file %s", _wf.name
                                )
                        except OSError as _we:
                            logger.warning(
                                "Sonarr Refiner: could not remove "
                                "stale work file %s (%s)", _wf.name, _we
                            )
            except OSError:
                pass
        elif dry and work_dir.is_dir():
            logger.info(
                "Sonarr Refiner: dry-run — skipping stale work-file "
                "cleanup in %s", work_dir
            )
        if not files:
            logger.info(
                "Sonarr Refiner: watched folder has no supported "
                "media files — nothing to do."
            )
            session.add(
                JobRunLog(
                    started_at=t_start,
                    ...
                    ok=True,
                    message=_refiner_job_log_text(
                        "Sonarr Refiner: watched folder has no "
        ...
        row.updated_at = utc_now_naive()
        detail = (
            f"Sonarr Refiner ({trigger}): processed={ok_c} "
            f"unchanged={noop_c} dry_run_items={dry_c} "
            f"cleanup_needed={cleanup_c} errors={err_c}"
        )
        job_lines = [detail]
