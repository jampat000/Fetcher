...
| **`.gitignore` hardened** | Added ignore rules for `.env`, `.env.*`, `fetcher-settings-backup*.json`, `*settings-backup*.json` (optional `!.env.example` if you add a template later). |
...
3. **Create a Release**  
   **Releases → Draft a new release** → tag (e.g. `v1.0.9`) → attach **`FetcherSetup.exe`** → Publish.
...
5. **Verify**  
   ...
   On Fetcher: **Settings → Software Updates** should reach GitHub without **404** (if `FETCHER_UPDATES_REPO` matches this repo).
