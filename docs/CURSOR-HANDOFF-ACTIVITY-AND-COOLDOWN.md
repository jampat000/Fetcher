...
- PR: https://github.com/jampat000/Fetcher/pull/new/chore/release-v1.0.20

### What v1.0.20 does
- Adds an `arr_action_log` history table.
- When Fetcher is about to call:
...
3. **Trimmer boomerang prevention (episode-level)**
...
2. Install/upgrade using the `FetcherSetup.exe` produced for `v1.0.20`.
3. Restart the **Fetcher Windows service** (or use WinSW restart).
4. Verify in the UI:
...
## Notes / assumptions
- If you are running the installed service (`FetcherSetup.exe`), edits in the git repo won’t affect it until you install a new release/installer.
...
2. **Cooldown was keyed by `action`** (`missing` vs `upgrade`) — the same movie could get **two** `MoviesSearch` calls in **one** Fetcher run if it appeared on both queues.
...
- Sample **`service/FetcherService.xml`** uses **`--host 0.0.0.0`** for LAN Web UI access (+ firewall / SECURITY notes).
